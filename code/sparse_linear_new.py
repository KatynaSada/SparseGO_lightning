...
class SparseLinearNew(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.training and self.dynamic and self.itr_count < self.Tend and self.itr_count%self.deltaT==0:
            ...
            # sparseMatrix = torch.sparse.FloatTensor( # DEPRECIATED
            #     self.indices,
            #     self.weights,
            #     torch.Size([self.out_features, self.in_features]),
            # )

            sparseMatrix = torch.sparse_coo_tensor(
                indices=self.indices,
                values=self.weights,
                size=[self.out_features, self.in_features],  # Use size instead of torch.Size
                dtype=torch.float32  # Specify the dtype if needed
            )
